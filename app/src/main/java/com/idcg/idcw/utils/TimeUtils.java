...
public class TimeUtils {
     ...
     * @return
     */
    public static String getTimesRemove3zero(String DATE1, String format) {
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(format);
        return String.valueOf(simpleDateFormat.format(Long.valueOf(DATE1)));
    }
    ...
    public static final String FORMAT_TYPE_4 = "yyyy/MM/dd HH:mm:ss";
...
//    不正规的返回时间格式
    ...
    public static String getOtherTime(String servertime) throws ParseException {

         String  date = servertime.replace("T", " ");//注意是空格+UTC
//        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//注意格式化的表达式

       String time= datatoLong(date,FORMAT_TYPE_1);


       String result=getTimeForDetailForCustom(time,FORMAT_TYPE_4);
        LogUtil.d("---时间-->",result);
        return  result;//getTimeForDetailForCustom(date,FORMAT_TYPE_4);

    }

    public  static String getTargetTime(String formattime){

        if(!TextUtils.isEmpty(formattime)){

            String temp="";

            temp=formattime.replace("-","/");
