 ...
 * .radius(10)//模糊半径
 * .blur();
 * @see <a href="https://github.com/pinguo-zhouwei/EasyBlur"></a>
 */
...
public class EasyBlur {
    ...
    public Bitmap blur(){
        ...
        if(SDK_INT > 8){// support library running on 2.3(API 9)  and higher 算法
            if(mPolicy == BlurPolicy.FAST_BLUR){
                LogUtil.d(TAG,"blur fast algorithm");
                return  fastBlur(mBitmap,mScale,mRadius);
            }else{
                LogUtil.d(TAG,"blur render script  algorithm");
    ...
    private static Bitmap rsBlur(Context context,Bitmap source,int radius,float scale){
        LogUtil.i(TAG,"origin size:"+source.getWidth()+"*"+source.getHeight());
        ...
        LogUtil.i(TAG,"scale size:"+inputBmp.getWidth()+"*"+inputBmp.getHeight());
